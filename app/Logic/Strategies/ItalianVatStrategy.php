...
class ItalianVatStrategy implements VatProcessingStrategyInterface
{
    ...
    protected const FLEXIBLE_VAT_REGEX = '/^\d{10,11}IT$/';
     ...
     * @return mixed It returns the VAT number as it is if valid or invalid, or a fixed version if possible, setting the last status accordingly.
     */
    public function process(string $vatNumber): mixed
    {
        ...
        // Lastly, try in case the format is flexible with the country code last
        if (preg_match(self::FLEXIBLE_VAT_REGEX, $vatNumber, $matches)) {
            $this->lastStatus = 'fixed';
            return self::COUNTRY_CODE . $matches[0]; // Prefix with IT for flexible format
        }
